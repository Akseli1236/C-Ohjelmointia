...
int main() {
    ...
    while ( true ) {
        ...
        string uppered;
        for (auto k: command_to_be_executed){
            uppered += toupper(k);
        ...
        vector<Command>::size_type i = 0;
        while (true) {
            if (COMMANDS[i].str == uppered){
                if (pieces.size() == 3){

                    if (string_to_double(pieces.at(1),test1) != 0 && string_to_double(pieces.at(2),test2) != 0){

                        cout << COMMANDS[i].action(test1,test2) << endl;
                    }else{
                        cout << "Error: a non-number operand." << endl;
                    }

                    break;
                }else if (pieces.size() == 1 && COMMANDS[i].parameter_number != 2){
