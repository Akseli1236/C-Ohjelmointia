                ...
                string fail;
                  for(const auto& pair: student_numbers){
                      ...
                      if (parts.at(1) == pair.first){
                          ...
                          for(auto number: new_phone){
                              if (not isdigit(number) && number != ' '){
                                  cout << "Erroneous phone number: " << new_phone << endl;
                                  cout << endl;
                                  fail = "yes";
                          ...
                          if (fail != "yes"){
                              pair.second->phone_number = new_phone;
                          }
                          ofstream file_obj(file_name);

                          file_obj.clear();

                          for(const auto& jep: user_ids){
                              file_obj << jep.second->student_number << ";" <<
                                          jep.second->user_id << ";" <<
                                          jep.second->name << ";" <<
                                          jep.second->phone_number << ";" <<
                                          jep.second->email << ";" << jep.second->skype << endl;
                          }
                          break;
                      }
                  }
